...
#ifndef _const_chre_slpi_handle
#define _const_chre_slpi_handle ((remote_handle)-1)
#endif //_const_chre_slpi_handle

static void _chre_slpi_pls_dtor(void* data) {
   remote_handle* ph = (remote_handle*)data;
   if(_const_chre_slpi_handle != *ph) {
      (void)__QAIC_REMOTE(remote_handle_close)(*ph);
      *ph = _const_chre_slpi_handle;
   }
}

static int _chre_slpi_pls_ctor(void* ctx, void* data) {
   remote_handle* ph = (remote_handle*)data;
   *ph = _const_chre_slpi_handle;
...
__QAIC_STUB_EXPORT remote_handle _chre_slpi_handle(void) {
   remote_handle* ph;
   if(adsp_pls_add_lookup) {
      if(0 == adsp_pls_add_lookup((uint32_t)_chre_slpi_handle, 0, sizeof(*ph),  _chre_slpi_pls_ctor, "chre_slpi",  _chre_slpi_pls_dtor, (void**)&ph))  {
         return *ph;
      }
      return (remote_handle)-1;
   } else if(HAP_pls_add_lookup) {
      if(0 == HAP_pls_add_lookup((uint32_t)_chre_slpi_handle, 0, sizeof(*ph),  _chre_slpi_pls_ctor, "chre_slpi",  _chre_slpi_pls_dtor, (void**)&ph))  {
         return *ph;
      }
      return (remote_handle)-1;
   }
...
uint32_t _chre_slpi_atomic_CompareAndExchange(uint32_t * volatile puDest, uint32_t uExchange, uint32_t uCompare);
...
__QAIC_STUB_EXPORT remote_handle _chre_slpi_handle(void) {
   static remote_handle handle = _const_chre_slpi_handle;
   if((remote_handle)-1 != handle) {
      return handle;
   } else {
      remote_handle tmp;
      int nErr = _chre_slpi_pls_ctor("chre_slpi", (void*)&tmp);
      if(nErr) {
         return (remote_handle)-1;
      }
      if(((remote_handle)-1 != handle) || ((remote_handle)-1 != (remote_handle)_chre_slpi_atomic_CompareAndExchange((uint32_t*)&handle, (uint32_t)tmp, (uint32_t)-1))) {
         _chre_slpi_pls_dtor(&tmp);
      }
      return handle;
   }
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(chre_slpi_skel_invoke)(uint32_t _sc, remote_arg* _pra) __QAIC_STUB_ATTRIBUTE {
   return __QAIC_REMOTE(remote_handle_invoke)(_chre_slpi_handle(), _sc, _pra);
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(chre_slpi_start_thread)(void) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 0;
   return _stub_method(_chre_slpi_handle(), _mid);
}
__QAIC_STUB_EXPORT int __QAIC_STUB(chre_slpi_wait_on_thread_exit)(void) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 1;
   return _stub_method(_chre_slpi_handle(), _mid);
}
__QAIC_STUB_EXPORT int __QAIC_STUB(chre_slpi_initialize_reverse_monitor)(void) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 2;
   return _stub_method(_chre_slpi_handle(), _mid);
}
__QAIC_STUB_EXPORT int __QAIC_STUB(chre_slpi_stop_thread)(void) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 3;
   return _stub_method(_chre_slpi_handle(), _mid);
}
