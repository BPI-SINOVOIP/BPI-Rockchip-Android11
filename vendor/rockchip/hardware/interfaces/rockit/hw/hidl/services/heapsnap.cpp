...
#define TAG                         "heapsnap"
#define VERION                      "v0.2"
#define HEPA_SNAP_PATH              "/data/vendor/test"
#define DEFAULT_HEAPSNAP_SIG        SIGUSR1
...
static mapinfo_t* parse_maps_line(char* line) {
  ...
  uintptr_t end;
  int name_pos;
  if (sscanf(line, "%08x-%08x %*4s %*x %*x:%*x %*d%n", &start,
             &end, &name_pos) < 2) {
    return NULL;
  }
...
static void dump_backtrace_symbols(FILE *fp, uintptr_t* frames, size_t frame_count) {
  for (size_t i = 0 ; i < frame_count; ++i) {
    ...
    if (symbol != NULL) {
      ...
      fprintf(fp, "          #%02zd  pc %08x %s (%s+%d)\n",
                        i, rel_pc, soname, best_name, frames[i] - offset);

      free(demangled_symbol);
    } else {
      fprintf(fp, "          #%02zd  pc %08x  %s\n",
                        i, rel_pc, soname);
    }
  }
}
