...
void rtw_stadel_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) ||
		...
		if (adapter->stapriv.asoc_sta_count == 1) { /* a sta + bc/mc_stainfo (not Ibss_stainfo) */
			...
			if (pwlan) {
				pwlan->fixed = _FALSE;
/* Fix bug41768 by borg 2014/04/07
 * Don't remove the network entry in scanned queue while receiving DeAuth or DisAssoc packet 
 * to prevent scan again in WPA after WPS success.
 */
#ifdef DHC_CONFIG_PATCH
				/* rtw_free_network_nolock(adapter, pwlan); */
#else
				rtw_free_network_nolock(adapter, pwlan);
#endif
