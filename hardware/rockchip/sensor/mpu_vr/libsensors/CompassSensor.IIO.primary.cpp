                    ...
                    mCoilsResetFd(0)
{
    ...
    if(!strcmp(dev_name, "USE_SYSFS")) {
        ...
        if(!strncmp(dev_full_name, "yas", 3)) {
            mYasCompass = true;
        } else if(!strcmp(dev_full_name, "akm8963")) {
            mAk8963Compass = true;
        }
    } else {
    ...
    if(mAk8963Compass) {
		/* TODO
		 * Configure AK8963 CNTL1 "BIT" bit
		 * Output data resolution
		 *  0: 14bit
		 *  1: 16bit
		 */
		write_sysfs_int(compassSysFs.compass_attr_1, 1);	// 16bit
    }
}
...
{
    VHANDLER_LOG;
    LOGI_IF(ENG_VERBOSE, "HAL:compass_fd=%d", compass_fd);
    return compass_fd;
}
...
int CompassSensor::inv_init_sysfs_attributes(void)
{
    ...
    if(mAk8963Compass) {
        sprintf(compassSysFs.compass_attr_1, "%s%s", sysfs_path, "/scale");
    }
#endif
...
int CompassSensor::isAk8963Compass(void)
{
    return mAk8963Compass;
}
