...
static int mixer_mode_set(struct stream_out *out)
{
     ...
     */
    if (out->device & AUDIO_DEVICE_OUT_HDMI_1) {
...
static void open_sound_card_policy(struct stream_out *out)
{
    ...
    if (support) {
        if(adev->dev_out.card != SND_OUT_SOUND_CARD_UNKNOWN) {
            ...
            if(adev->outputs[OUTPUT_HDMI_MULTI] != NULL) {
                out->device &= ~AUDIO_DEVICE_OUT_HDMI_1;
            } else {
                out->device |= AUDIO_DEVICE_OUT_HDMI_1;
            }
        }
    ...
    audio_devices_t devices = (AUDIO_DEVICE_OUT_HDMI_1|AUDIO_DEVICE_OUT_SPEAKER);
...
static int start_output_stream(struct stream_out *out)
{
    ...
    if (out->device & AUDIO_DEVICE_OUT_HDMI_1) {
        audio_devices_t route_device = out->device & AUDIO_DEVICE_OUT_HDMI_1;
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        if ((val != 0) && ((out->device & val) != val) ||
            (val != 0) && !(out->device & AUDIO_DEVICE_OUT_HDMI_1)) {
...
static void check_hdmi_reconnect(struct stream_out *out)
{
  ...
  //      reset_bitstream_buf(out);
     ...
     */
    if (out->snd_reopen && is_bitstream(out) && (out->device == AUDIO_DEVICE_OUT_HDMI_1)) {
#ifdef USE_DRM
...
static int bitstream_write_data(struct stream_out *out, void* buffer, size_t bytes)
{
    ...
    struct audio_device *adev = out->dev;
    int ret = 0;
    if ((out->device & AUDIO_DEVICE_OUT_HDMI_1) && (is_multi_pcm(out) || is_bitstream(out))) {
...
false_alarm:
    ...
    if ((out->device & AUDIO_DEVICE_OUT_HDMI_1) && is_bitstream(out)) {
                                   ...
                                   const char *address __unused)
{
    ...
    if(devices & AUDIO_DEVICE_OUT_HDMI_1) {
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        if(device == (int)AUDIO_DEVICE_OUT_HDMI_1){
            struct stream_out *out = adev->outputs[OUTPUT_HDMI_MULTI];
            if((out != NULL) && is_bitstream(out) && (out->device == AUDIO_DEVICE_OUT_HDMI_1)) {
